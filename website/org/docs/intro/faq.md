...
- 打包工具不同
  - Amis 使用 Fis3 作为开发工具
  - Ovine 封装了 Webpack 作为开发工具
- 使用方式不一样
  - Amis 需要自己搭建一系列开发环境，官方虽然提供了 [Amis-Admin](https://github.com/fex-team/amis-admin) 一个 Demo 启动项目，不过很基础，要实现 Ovine 类似功能，都添加很多逻辑
  - Ovine 集成了管理系统常用功能的封装，并且开箱即用，远离搭建开发环境的烦恼
...
Ovine 目前不支持多语言切换，等 Amis 支持多语言后，我们将第一时间支持多语言环境。

### 是否考虑添加可视化拖拽 UI 编辑器？

目前已经有开发 UI 编辑器的计划。从而实现快速的开发 Admin 系统 UI 界面，远离繁杂的配置项。如果你对 UI 编辑器关注，可经常查看 Ovine 源码查看进度，或者 加入 Q 群讨论～
