...
class Subscriber {
   ...
   */
  constructor(collector: any, onChange: any, collectOnCreate: any = false) {
...
export type SubscriberAndCallbacksFor<
  ...
  Q extends QueryMethods | null = any
> = {
...
export type QueryMethods<S = any, O = any, R extends MethodRecordBase<S> = any> = (
  state?: S,
  options?: O
) => R

export type QueryCallbacksFor<M extends QueryMethods | null> = M extends QueryMethods<
  any,
  any,
  infer R
>
...
export function useMethods<S, R extends MethodRecordBase<S>, Q extends QueryMethods>(
  ...
  const [reducer, methodsFactory] = useMemo(() => {
    ...
    if (isFunction(methodsOrOptions)) {
  ...
  return useMemo(
    () => ({
      getState,
      subscribe: (collector: any, cb: any, collectOnCreate: any) =>
...
export function createQuery<Q extends QueryMethods>(queryMethods: Q, getState: any) {
